...
add_safe_globals(['data_utils.Corpus'])
...
class STSBIterator(object):
    def __init__(self, data, bsz):
        ...
        self.encoded = data[0]
        self.labels = data[1] # FloatTensor
    ...
    def get_batch(self, index_list):
        ...
        # mask_idx = pad_sequence(mask_idx)
        sublabels = torch.FloatTensor(sublabels)  # STSB uses float labels
...
class Corpus(object):
    def __init__(self, path, dataset, *args, **kwargs):
        ...
        elif self.dataset == 'stsb':
            self.vocab.count_stsb(os.path.join(path, 'train.tsv'), add_cls_token=True)
            self.vocab.count_stsb(os.path.join(path, 'dev.tsv'), add_cls_token=True)
            self.vocab.count_stsb(os.path.join(path, 'test.tsv'), add_cls_token=True)

        elif self.dataset == 'qqp':
            self.vocab.count_qqp(os.path.join(path, 'train.tsv'), add_cls_token=True)
            self.vocab.count_qqp(os.path.join(path, 'dev.tsv'), add_cls_token=True)
            if os.path.exists(os.path.join(path, 'test.tsv')):
                self.vocab.count_qqp(os.path.join(path, 'test.tsv'), add_cls_token=True)
        elif self.dataset == 'qnli':
            self.vocab.count_qnli(os.path.join(path, 'train.tsv'), add_cls_token=True)
            self.vocab.count_qnli(os.path.join(path, 'dev.tsv'), add_cls_token=True)
            if os.path.exists(os.path.join(path, 'test.tsv')):
                self.vocab.count_qnli(os.path.join(path, 'test.tsv'), add_cls_token=True)
        elif self.dataset == 'rte':
            self.vocab.count_rte(os.path.join(path, 'train.tsv'), add_cls_token=True)
            self.vocab.count_rte(os.path.join(path, 'dev.tsv'), add_cls_token=True)
            if os.path.exists(os.path.join(path, 'test.tsv')):
                self.vocab.count_rte(os.path.join(path, 'test.tsv'), add_cls_token=True)
        ...
        if self.dataset in ['ptb', 'wt2', 'wt103']:
            ...
            self.train = self.vocab.encode_stsb_file(
                os.path.join(path, 'train.tsv'), add_cls_token=True)
            self.valid = self.vocab.encode_stsb_file(
                os.path.join(path, 'dev.tsv'), add_cls_token=True)
            # Note: STSB test set usually has no labels, or is in a different file
            if os.path.exists(os.path.join(path, 'test.tsv')):
                self.test = self.vocab.encode_stsb_file(
                    os.path.join(path, 'test.tsv'), add_cls_token=True)
        elif self.dataset == 'qqp':
            self.train = self.vocab.encode_qqp_file(
                os.path.join(path, 'train.tsv'), add_cls_token=True)
            self.valid = self.vocab.encode_qqp_file(
                os.path.join(path, 'dev.tsv'), add_cls_token=True)
            # QQP test set usually has labels
            if os.path.exists(os.path.join(path, 'test.tsv')):
                self.test = self.vocab.encode_qqp_file(
                    os.path.join(path, 'test.tsv'), add_cls_token=True)
        elif self.dataset == 'qnli':
            self.train = self.vocab.encode_qnli_file(
                os.path.join(path, 'train.tsv'), add_cls_token=True)
            self.valid = self.vocab.encode_qnli_file(
                os.path.join(path, 'dev.tsv'), add_cls_token=True)
            # QNLI test set usually has labels
            if os.path.exists(os.path.join(path, 'test.tsv')):
                self.test = self.vocab.encode_qnli_file(
                    os.path.join(path, 'test.tsv'), add_cls_token=True)
        elif self.dataset == 'rte':
            self.train = self.vocab.encode_rte_file(
                os.path.join(path, 'train.tsv'), add_cls_token=True)
            self.valid = self.vocab.encode_rte_file(
                os.path.join(path, 'dev.tsv'), add_cls_token=True)
            # RTE test set usually has labels
            if os.path.exists(os.path.join(path, 'test.tsv')):
                self.test = self.vocab.encode_rte_file(
                    os.path.join(path, 'test.tsv'), add_cls_token=True)
    ...
    def get_iterator(self, split, *args, **kwargs):
        ...
        if split == 'train':
            if self.dataset in ['ptb', 'wt2', 'wt103', 'enwik8', 'text8']:
                ...
                data_iter = SST2Iterator(self.train, *args, **kwargs)  # Reuse SST2Iterator since both are binary classification
            elif self.dataset == 'qnli':
                data_iter = SST2Iterator(self.train, *args, **kwargs)  # Reuse SST2Iterator since both are binary classification
            elif self.dataset == 'stsb':
                data_iter = STSBIterator(self.train, *args, **kwargs)
            elif self.dataset == 'rte':
                data_iter = SST2Iterator(self.train, *args, **kwargs)  # Reuse SST2Iterator since both are binary classification
            ...
            if self.dataset in ['ptb', 'wt2', 'wt103', 'enwik8', 'text8']:
                ...
                data_iter = SST2Iterator(data, *args, **kwargs)  # Reuse SST2Iterator since both are binary classification
            elif self.dataset == 'qnli':
                data_iter = SST2Iterator(data, *args, **kwargs)  # Reuse SST2Iterator since both are binary classification
            elif self.dataset == 'stsb':
                data_iter = STSBIterator(data, *args, **kwargs)
            elif self.dataset == 'rte':
                data_iter = SST2Iterator(data, *args, **kwargs)  # Reuse SST2Iterator since both are binary classification
...
def get_lm_corpus(datadir, dataset):
    ...
    if os.path.exists(fn):
        ...
        elif dataset in ['csqa', 'sst2', 'sst2_v2', 'stsb', 'qqp', 'qnli', 'rte']:
            kwargs['special'] = ['<eos>']
