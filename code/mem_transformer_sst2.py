...
# 导入所需的gate类以避免eval()调用时的NameError
try:
    ...
    # 如果导入失败，设置为None，后面会有默认处理
    GlobalRouterGate = None
    EnhancedGlobalRouterGate = None
    CustomNaiveGate_Balance = None
    SwitchGate = None

def resolve_gate_name(gate_name_str):
    """安全地解析gate名称字符串为实际的类"""
    if isinstance(gate_name_str, str):
        # 创建一个本地字典，包含所有可能的gate类
        gate_dict = {
            'NaiveGate': NaiveGate,
        ...
        # 如果gate_class是None（导入失败），使用NaiveGate作为后备
        return gate_class if gate_class is not None else NaiveGate
    else:
        return gate_name_str or NaiveGate
    ...
    def __init__(self, n_head, d_model, d_head, d_inner, dropout, **kwargs):
        ...
        if kwargs.get('moe') is False and kwargs.get('dense_drop') is False:
            ...
            self.pos_ff = CustomizedMoEPositionwiseFF(d_model, d_inner, dropout,
                                        ...
                                        gate_name=resolve_gate_name(kwargs.get('gate_name')), 
                                        hyper_size=kwargs.get('hyper_size'),
...
class MemTransformerLM(nn.Module):
    def __init__(self, n_token, n_layer, n_head, d_model, d_head, d_inner,
                 ...
                 dense_drop=False, expert_drop=0.5, num_expert=64, hyper_size=None, attn_moe=False, ablation_args=None):
            ...
            for i in range(n_layer):
                ...
                self.layers.append(
                    RelPartialLearnableDecoderLayer(
                        ...
                        hyper_size=hyper_size, args=ablation_args)
                )
