...
parser.add_argument('--dataset', type=str, default='wt103',
                    choices=['wt103', 'lm1b', 'enwik8', 'text8', 'csqa', 'sst2', 'sst2_v2', 'stsb', 'qqp', 'qnli', 'rte'],
                    help='dataset name')
...
parser.add_argument('--hyper_size', type=int, default=None,
                    help='Hidden size of hyper router')
...
## SMoE Specific Arguments
parser.add_argument('--switch_eps', type=float, default=0.1,
                    help='Noise epsilon for Switch routing exploration during training')
parser.add_argument('--capacity_factor', type=str, default='(1.2,2.4)',
                    help='Capacity factor for Switch routing as a tuple (train,eval)')
...
if args.restart:
    ...
    # Check if we need to use RMoE transformer
    model = MemTransformerLM(ntokens, args.n_layer, args.n_head, args.d_model,
        ...
        dense_drop=args.dense_drop, expert_drop=args.expert_drop, num_expert=args.num_expert, attn_moe=args.attn_moe, ablation_args=args)
    model.apply(weights_init)
    model.word_emb.apply(weights_init) # ensure embedding init is not overridden by out_layer in case of weight sharing
    

    if (hasattr(model, 'router_type') and model.router_type in ['gru', 'rnn']) or \
       (args.gate_name == 'SMoEGate' and args.moe):
        model.classifier = nn.Linear(args.d_model, 2)  # SST2 二分类
...
with open(args.pretrained_weight, 'rb') as f:
    pretrained_model = torch.load(f, weights_only=False)


if hasattr(pretrained_model, 'state_dict'):

    pretrained_model_checkpoint = pretrained_model.state_dict()
    logging('Loaded model object, extracting state_dict')
else:
   
    pretrained_model_checkpoint = pretrained_model
    logging('Loaded state_dict directly')


def convert_hyper_router_weights(pretrained_checkpoint, target_model_state):
    """
    Convert HyperRouterGate weights to regular Gate weights
    """
    converted_checkpoint = {}
    
    for key in pretrained_checkpoint.keys():
        if key in target_model_state:
            if pretrained_checkpoint[key].shape == target_model_state[key].shape:
                converted_checkpoint[key] = pretrained_checkpoint[key]
            else:
                logging('Can not load {}, shape do not match'.format(key))
        elif 'gate.embedding' in key or 'gate.hypernet' in key:
            # Skip HyperRouter specific weights
            logging('Skipping HyperRouter component: {}'.format(key))
        else:
            logging('Can not load {}'.format(key))
    
   
    if args.gate_name == 'HyperRouterGate':
        logging('Converting HyperRouter weights to regular Gate weights...')
        
        for layer_idx in range(args.n_layer):
            
            if args.moe_index is not None:
                moe_indices = list(map(int, args.moe_index.split(',')))
                if layer_idx not in moe_indices:
                    continue
            
           
            embedding_key = f'layers.{layer_idx}.pos_ff.gate.embedding'
            hypernet_weight_key = f'layers.{layer_idx}.pos_ff.gate.hypernet.0.weight'
            hypernet_bias_key = f'layers.{layer_idx}.pos_ff.gate.hypernet.0.bias'
            hypernet_final_weight_key = f'layers.{layer_idx}.pos_ff.gate.hypernet.2.weight'
            hypernet_final_bias_key = f'layers.{layer_idx}.pos_ff.gate.hypernet.2.bias'
            
            
            gate_weight_key = f'layers.{layer_idx}.pos_ff.gate.gate.weight'
            gate_bias_key = f'layers.{layer_idx}.pos_ff.gate.gate.bias'
            

            required_keys = [embedding_key, hypernet_weight_key, hypernet_bias_key, 
                           hypernet_final_weight_key, hypernet_final_bias_key]
            if all(key in pretrained_checkpoint for key in required_keys) and gate_weight_key in target_model_state:
                try:
                    
                    embedding = pretrained_checkpoint[embedding_key]
                    ...
                    with torch.no_grad():
                        ...
                        h1_weight = pretrained_checkpoint[hypernet_weight_key].cpu()
                        h1_bias = pretrained_checkpoint[hypernet_bias_key].cpu()
                        h1_out = torch.mm(embedding, h1_weight.t()) + h1_bias
                     
                        h1_out = torch.relu(h1_out)
                        
                       
                        h2_weight = pretrained_checkpoint[hypernet_final_weight_key].cpu()
                        h2_bias = pretrained_checkpoint[hypernet_final_bias_key].cpu()
                        hypernet_outputs = torch.mm(h1_out, h2_weight.t()) + h2_bias
                        
                        num_expert = args.moe_num_expert
                        d_model = args.d_model
                        expected_size = num_expert * (d_model + 1)
                        
                     
                        if hypernet_outputs.numel() != expected_size:
                            logging(f'Warning: hypernet output size {hypernet_outputs.numel()} != expected {expected_size}')
                           
                            hypernet_outputs = hypernet_outputs.view(-1)[:expected_size].view(num_expert, -1)
                        else:
                            hypernet_outputs = hypernet_outputs.view(num_expert, -1)
                        
                       
                        if hypernet_outputs.size(1) == d_model + 1:
                            gate_weight = hypernet_outputs[:, :-1]  # (num_expert, d_model)
                            gate_bias = hypernet_outputs[:, -1]     # (num_expert,)
                        else:
                            
                            gate_weight = hypernet_outputs[:, :d_model]
                            gate_bias = torch.zeros(num_expert)
                            logging(f'Warning: Using zero bias for layer {layer_idx}')
                        
                       
                        target_weight_shape = target_model_state[gate_weight_key].shape
                        target_bias_shape = target_model_state[gate_bias_key].shape
                        
                        if gate_weight.shape == target_weight_shape and gate_bias.shape == target_bias_shape:
                            converted_checkpoint[gate_weight_key] = gate_weight
                            converted_checkpoint[gate_bias_key] = gate_bias
                            logging(f'Successfully converted HyperRouter weights for layer {layer_idx}')
                        else:
                            logging(f'Shape mismatch for layer {layer_idx}: gate_weight {gate_weight.shape} vs {target_weight_shape}, gate_bias {gate_bias.shape} vs {target_bias_shape}')
                        
                except Exception as e:
                    logging(f'Failed to convert HyperRouter weights for layer {layer_idx}: {e}')
                    import traceback
                    logging(traceback.format_exc())
            else:
                missing_keys = [key for key in required_keys if key not in pretrained_checkpoint]
                if missing_keys:
                    logging(f'Missing HyperRouter keys for layer {layer_idx}: {missing_keys}')
    
    return converted_checkpoint


def filter_compatible_weights(pretrained_checkpoint, target_model_state):
    """
    Filter pretrained weights that are compatible with target model
    """
    filtered_checkpoint = {}
    ...
    for key in pretrained_checkpoint.keys():
        if key in target_model_state:
            if pretrained_checkpoint[key].shape == target_model_state[key].shape:
                filtered_checkpoint[key] = pretrained_checkpoint[key]
                logging('Loaded: {}'.format(key))
            else:
                logging('Can not load {}, shape do not match. Pretrained: {} vs Target: {}'.format(
                    key, pretrained_checkpoint[key].shape, target_model_state[key].shape))
        else:
            logging('Can not load {}, key not found in target model'.format(key))
    

    total_pretrained = len(pretrained_checkpoint)
    total_loaded = len(filtered_checkpoint)
    total_target = len(target_model_state)
    
    logging(f'Weight loading summary:')
    logging(f'  - Pretrained model parameters: {total_pretrained}')
    logging(f'  - Target model parameters: {total_target}')
    logging(f'  - Successfully loaded: {total_loaded}')
    logging(f'  - Loading rate: {total_loaded/total_target*100:.1f}%')
    
    return filtered_checkpoint


if args.gate_name == 'HyperRouterGate':
    filtered_checkpoint = convert_hyper_router_weights(pretrained_model_checkpoint, model.state_dict())
else:

    filtered_checkpoint = filter_compatible_weights(pretrained_model_checkpoint, model.state_dict())
        ...
        for i, (data, mask, label) in enumerate(eval_iter):
            ...
            if (hasattr(model, 'router_type') and model.router_type in ['gru', 'rnn']) or \
               (args.gate_name == 'SMoEGate' and args.moe):
                ...
                base_model = para_model.module if hasattr(para_model, 'module') else para_model
                
             
                forward_result = base_model._forward(data, mems=mems[0] if mems else None)
                if len(forward_result) == 4:
              
                    hidden_states, new_mems, _, _ = forward_result
                else:
                  
                    hidden_states, new_mems = forward_result
                ...
                if not hasattr(base_model, 'classifier'):
                    base_model.classifier = nn.Linear(args.d_model, 2).to(device)  # 二分类
                
                
                predict = base_model.classifier(last_hidden)
                mems = [new_mems] if new_mems is not None else mems
            else:
...
def train():
    ...
    for batch, (data, mask, label) in enumerate(train_iter):
        ...
        # Add load balance loss if enabled
        if args.load_balance > 0:
            balance_loss = 0
            for name, m in model.named_modules():
                if hasattr(m, 'loss') and 'gate' in name.lower():
                    if m.loss is not None:
                        balance_loss += m.loss
            if hasattr(balance_loss, 'item') and balance_loss.item() > 0:
                loss += args.load_balance * balance_loss
        
        # Add RMoE balance loss if enabled
        if args.moe_router_type in ['gru', 'rnn'] and args.balance_loss_weight > 0:
            rmoe_balance_loss = 0
            for name, m in model.named_modules():
                if hasattr(m, 'router_balance_loss') and m.router_balance_loss is not None:
                    rmoe_balance_loss += m.router_balance_loss
                # Also check for standard balance loss in RMoE gates
                elif 'rmoe' in name.lower() and hasattr(m, 'loss') and m.loss is not None:
                    rmoe_balance_loss += m.loss
            if hasattr(rmoe_balance_loss, 'item') and rmoe_balance_loss.item() > 0:
                loss += args.balance_loss_weight * rmoe_balance_loss
