...
def set_router_mode(model, args, flag=True):
    ...
    if not hasattr(set_router_mode, "_printed_mode"):
        set_router_mode._printed_mode = False
        
    if not set_router_mode._printed_mode or flag != getattr(set_router_mode, "_last_flag", None):
        print('** Using Score-Based Average for Dense Inference')
        set_router_mode._printed_mode = True
        set_router_mode._last_flag = flag
            ...
            if hasattr(m, 'top_k') and hasattr(m, 'gate'):
                if isinstance(m.gate, BaseGate) and not isinstance(m.gate, CustomNaiveGate_Attn):
                    if flag:
                        ...
                        target_top_k = min(args.moe_num_expert, 16)  # 限制最大专家数为16，避免OOM
                        m.top_k = target_top_k
                        m.gate.top_k = target_top_k
                    else:
                    ...
                    if layer_count <= 4:
                        print(f'Set {name}, Top-K = {m.top_k} {m.gate.top_k}')
                        
                    
                    if time.time() - start_time > timeout_seconds:
                        print(f"Warning: set_router_mode timeout after processing {layer_count} layers.")
                        break
    
    except Exception as e:
        print(f"Error in set_router_mode: {e}")
 
        if current_gate == 0:
            current_gate = args.moe_top_k if not flag else min(args.moe_num_expert, 16)
