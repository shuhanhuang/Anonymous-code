...
from custom_glb_gate import * 
from custom_glb_gate import GlobalRouterGate, EnhancedGlobalRouterGate, CustomNaiveGate_Balance
...
from custom_glb_transformer import FMoETransformerMLP
from custom_glb_gate import *
...
class CustomizedMoEPositionwiseFF(FMoETransformerMLP):
    def __init__(self, d_model, d_inner, dropout, pre_lnorm=False, moe_num_expert=64, moe_top_k=2, gate_name=NaiveGate, hyper_size=None, gate_kwargs=None):
        activation = nn.Sequential(
            nn.ReLU(),
            nn.Dropout(dropout)
        )
        super().__init__(num_expert=moe_num_expert, d_model=d_model, d_hidden=d_inner, top_k=moe_top_k,
                activation=activation, gate=gate_name, hyper_size=hyper_size, gate_kwargs=gate_kwargs)
    ...
    def forward(self, inp):
        ...
        if len(original_shape) == 3:
            ...
            seq_len, batch_size, d_model = original_shape
            if hasattr(self.gate, 'set_batch_info') and callable(self.gate.set_batch_info):
                self.gate.set_batch_info(batch_size, seq_len)
...
class DecoderLayer(nn.Module):
    def __init__(self, n_head, d_model, d_head, d_inner, dropout, **kwargs):
        ...
        if kwargs.get('moe') is False and kwargs.get('dense_drop') is False:
            ...
            gate_name_str = kwargs.get('gate_name')
            if isinstance(gate_name_str, str):
                
                gate_dict = {
                    'NaiveGate': NaiveGate,
                    'GlobalRouterGate': GlobalRouterGate,
                    'EnhancedGlobalRouterGate': EnhancedGlobalRouterGate,
                    'CustomNaiveGate_Balance': CustomNaiveGate_Balance,
                    'SwitchGate': SwitchGate,
                }
                gate_class = gate_dict.get(gate_name_str, NaiveGate)
            else:
                gate_class = gate_name_str or NaiveGate
            ...
            self.pos_ff = CustomizedMoEPositionwiseFF(d_model, d_inner, dropout,
                                        ...
                                        gate_name=gate_class, hyper_size=kwargs.get('hyper_size'),
                                        gate_kwargs=kwargs.get('gate_kwargs'))
...
class MemTransformerLM(nn.Module):
    def __init__(self, n_token, n_layer, n_head, d_model, d_head, d_inner,
                 ...
                 dense_drop=False, expert_drop=0.5, num_expert=64, attn_moe=False, ablation_args=None):
        ...
        # Store moe_index as instance attribute for later use
        self.moe_index = moe_index
        ...
        if attn_type == 0: # the default attention
            for i in range(n_layer):
                ...
                gate_kwargs = {
                    'num_decoder_layers': n_layer,  # 确保gate知道总层数
                }
                
                if gate_name in ['GlobalRouterGate', 'EnhancedGlobalRouterGate']:
                    if ablation_args is not None:
                        gate_kwargs['args'] = ablation_args
                    else:
                        try:
                            import sys
                            args = sys.modules['__main__'].args
                            gate_kwargs['args'] = args
                        except:
                            pass
                ...
                self.layers.append(
                    RelPartialLearnableDecoderLayer(
                        ...
                        moe=layer_moe, moe_num_expert=moe_num_expert, moe_top_k=moe_top_k, 
                        gate_name=gate_name, gate_kwargs=gate_kwargs,
                        dense_drop=layer_dense_drop, expert_drop=expert_drop, num_expert=num_expert, moe_attn=attn_moe, args=ablation_args)
                )
            ...
            for i in range(n_layer):
                self.layers.append(
                    RelLearnableDecoderLayer(
                        ...
                        dense_drop=layer_dense_drop, expert_drop=expert_drop, num_expert=num_expert, args=ablation_args)
                )
    ...
    def _forward(self, dec_inp, mems=None):
        ...
        for i, layer in enumerate(self.layers):
            if hasattr(layer, 'pos_ff') and hasattr(layer.pos_ff, 'gate'):
                ...
                if hasattr(gate, 'clear_history') and callable(gate.clear_history):
                    gate.clear_history()
                if hasattr(gate, 'set_batch_info') and callable(gate.set_batch_info):
                    gate.set_batch_info(bsz, qlen)
        ...
        if self.attn_type == 0: # default
            ...
            for i, layer in enumerate(self.layers):
                if hasattr(layer, 'pos_ff') and hasattr(layer.pos_ff, 'gate'):
                    gate = layer.pos_ff.gate
                    if hasattr(gate, 'set_layer_index') and callable(gate.set_layer_index):
                        gate.set_layer_index(i)
                ...
                if hasattr(layer, 'pos_ff') and hasattr(layer.pos_ff, 'gate'):
                    gate = layer.pos_ff.gate
                    if hasattr(gate, 'add_to_history') and callable(gate.add_to_history):
                        gate.add_to_history(core_out)
    ...
    def forward(self, data, target, *mems, collect_layer_stats=False):
        ...
        if collect_layer_stats:
            ...
            if not hasattr(self, 'moe_index'):
                ...
                for i in range(self.n_layer):
                    ...
                    if hasattr(layer, 'pos_ff') and hasattr(layer.pos_ff, 'gate'):
                        gate = layer.pos_ff.gate
                        if hasattr(gate, 'num_expert') or hasattr(gate, 'num_experts') or hasattr(gate, 'top_k'):
                            moe_layers.append(i)
                self.moe_index = moe_layers
            
            for i in range(self.n_layer):
                is_moe_layer = False
                try:
                    is_moe_layer = i in self.moe_index
                except (TypeError, AttributeError):
                    layer = self.layers[i]
                    if hasattr(layer, 'pos_ff') and hasattr(layer.pos_ff, 'gate'):
                        gate = layer.pos_ff.gate
                        is_moe_layer = hasattr(gate, 'num_expert') or hasattr(gate, 'num_experts') or hasattr(gate, 'top_k')
                ...
                if is_moe_layer:
                    ...
                    if hasattr(layer, 'pos_ff') and hasattr(layer.pos_ff, 'gate'):
                        ...
                        if hasattr(gate, 'last_expert_indices') and hasattr(gate, 'last_expert_weights'):
                            indices = gate.last_expert_indices
                            weights = gate.last_expert_weights
                        elif hasattr(gate, 'last_routing_indices') and hasattr(gate, 'last_routing_weights'):
                            indices = gate.last_routing_indices
                            weights = gate.last_routing_weights
                        
                        if indices is not None and weights is not None:
                            layer_info.append((indices, weights))
                        else:
                            layer_info.append((None, None))
                    else:
                      
                        layer_info.append((None, None))
            
            return [loss, layer_info] + list(new_mems)
        else:
            return [loss] + list(new_mems)
